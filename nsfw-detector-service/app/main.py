...
async def lifespan(app: FastAPI):
    setup_logging()
    logger = get_logger("nsfw-detector.main")
    ...
    logger.info("Starting nsfw-detector-service (port=%s)", settings.PORT)
    try:
        model_manager.load()
    ...
    logger.info("Shutting down nsfw-detector-service")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        ...
        version="1.0.0",
        description="NSFW Image Detection API (Marqo/nsfw-image-detection-384)",
        lifespan=lifespan,
